...
import { showModal, hideModal, updateInfoPanel, deselectObject as deselectObjectUI, showMessage, showLeaderboard, openQuizEditor, hideFocusView, showFocusView, updateSettingsUI, startQuiz, saveScoreToLeaderboard } from './ui-handler.js';
...
export const drawingColors = ['#000000', '#ef4444', '#f97316', '#eab308', '#22c55e', '#3b82f6', '#8b5cf6', '#ffffff'];
...
export function getRoomGroups() { return roomGroups; }
...
export function listenToMuseumData(db, museumId, museumDataListener, isViewerMode, currentUser, goBackToDashboard) {
    if (museumDataListener) museumDataListener(); // Unsubscribe from previous listener
    ...
    // The onValue function returns the unsubscribe function, which we store.
...
async function loadMuseumState(state) {
    ...
    if (isViewerMode) {
        if (currentMusic && currentMusic !== 'none') {
            ...
            // updateMuteButtonUI(); // This function needs to be in ui-handler
        } else {
...
function setup3DEventListeners() {
    ...
    pointerLockControls.addEventListener('unlock', () => {
        if (!isViewerMode) {
            if (!isInteractingWithUI) switchToEditMode(); else viewerOverlay.style.display = 'flex';
...
function onCanvasMouseDown(event) {
    ...
    if (isEditMode) {
        ...
        if (controlIntersects.length > 0) {
             ...
             if (action === 'quiz') { openQuizEditor(room, db, getCurrentMuseumId()); return; }
             if (action === 'add') {
        ...
        if (quizIntersect) {
            ...
            if (room && room.userData.quiz) {
                startQuiz(room, getCurrentUser());
            }
            return;
        }
...
function deleteRoom(roomToDelete) {
    ...
    artworksToRemove.forEach(obj => {
        if (obj.userData.isDrawingCanvas) {
            ...
            if(listener) listener(); // Detach listener
